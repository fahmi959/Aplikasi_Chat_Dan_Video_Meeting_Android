package com.capstonebangkit.aplikasi_fahmi_chat
...
import com.capstonebangkit.aplikasi_fahmi_chat.databinding.ItemMessageBinding
...
class FirebaseMessageAdapter(
    ...
    private fun showDeleteConfirmationDialog(position: Int) {
        ...
        dialogBuilder.setMessage("Hapus pesan ini?")
            .setPositiveButton("Hapus") { dialog, _ ->
                val messageRef = getRef(position)
                val message = getItem(position)

                val allowedEmails = listOf("fahmiardiansyah959@gmail.com", "fahmiardiansyah959@students.unnes.ac.id", "fahmiardiansyah161201@gmail.com")
                val currentUserEmail = FirebaseAuth.getInstance().currentUser?.email

                if (message != null && allowedEmails.contains(currentUserEmail)) {
                    messageRef.removeValue().addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            Toast.makeText(context, "Pesan berhasil dihapus", Toast.LENGTH_SHORT).show()
                        } else {
                            Toast.makeText(context, "Gagal menghapus pesan", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    Toast.makeText(context, "Anda tidak diizinkan menghapus pesan ini", Toast.LENGTH_SHORT).show()
                }
